...
static void dssmgr_oly_mirror(struct dssmgr_oly_data *data, uint8_t s_idx)
{
	...
	if (data->mirror.src_idx != DSSMGR_OLY_INVALID_IDX
		 ...
		 * like width, height, etc.  Not needed now since we
		...
		if (rc != 0) {
			...
			dssmgr_oly_disable_oly(data, d_idx);
			...
			if (rc != 0) {
				...
				oly = data->olys[d_idx];
				dev = oly->manager->device;
				rc = oly->manager->apply(oly->manager);
				if (rc == 0 && dev && dev->driver
				   && dev->driver->update) {
					rc = dev->driver->update(dev, 0, 0,
						dev->panel.timings.x_res,
						dev->panel.timings.y_res);
			...
			mirror->dst_x = d_info.pos_x;
			mirror->dst_y = d_info.pos_y;
			mirror->dst_w = d_info.out_width;
			mirror->dst_h = d_info.out_height;
		}
				...
				struct omap_overlay_info *info)
{
	...
	if (scale == DSSMGR_SCALE_FIT_TO_MIRROR) {
		 ...
		 */
		dssmgr_oly_non_square_pixel_chk(in_h, out_w, out_h, &in_w);

		/* Only for FIT_TO_MIRROR overlays (just the video/cam oly),
		 * if mirroring is disabled use a different scaling type.
		 */
		if (mirror->src_idx == DSSMGR_OLY_INVALID_IDX)
