...
{
	struct bus_type *bus = dss_get_bus();
	struct omap_writeback *writeback = omap_dss_get_wb(0);

	/* Ensure Writeback Path is Reset.  If interrupt completes
	   after this, than these values are overwritten.  If interrupt,
	   is lost, than this will reset the values so they'll be valid
	   on resume */
