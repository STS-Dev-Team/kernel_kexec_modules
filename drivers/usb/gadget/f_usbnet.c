...
static void usbnet_send_uevent(int config)
{
	...
	pr_info("Sending USBLAN %s uevent \n",
...
static int usb_ether_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (!req) {
		pr_err("%s: could not obtain tx request\n", __func__);
		return 1;
	}
	...
	if (rc != 0) {
		...
		pr_err("%s: could not queue tx request\n", __func__);
	}
...
#endif
{
	...
	if (!ep) {
		pr_err("%s auto-configure hs_bulk_in_desc error\n",
			__func__);
		goto autoconf_fail;
	}
	...
	if (!ep) {
		pr_err("%s auto-configure hs_bulk_out_desc error\n",
		      __func__);
		goto autoconf_fail;
	}
	ep->driver_data = g_usbnet_context;
	...
	if (!ep) {
		pr_err("%s auto-configure hs_intr_out_desc error\n",
		      __func__);
		goto autoconf_fail;
	}
	ep->driver_data = g_usbnet_context;
	...
	for (n = 0; n < MAX_BULK_RX_REQ_NUM; n++) {
		req = usb_ep_alloc_request(g_usbnet_context->bulk_out,
					 GFP_KERNEL);
		if (!req) {
			pr_err("%s: alloc request bulk_out fail\n",
				__func__);
			break;
		}
	...
	for (n = 0; n < MAX_BULK_TX_REQ_NUM; n++) {
		req = usb_ep_alloc_request(g_usbnet_context->bulk_in,
					 GFP_KERNEL);
		if (!req) {
			pr_err("%s: alloc request bulk_in fail\n",
				__func__);
			break;
		}
...
static void do_set_config(u16 new_config)
{
	...
	if (new_config == 1) { /* Enable End points */
		...
		if (result != 0) {
			pr_err("%s:  failed to enable BULK_IN EP ret=%d\n",
			      __func__, result);
		}
		...
		if (result != 0) {
			pr_err("%s:  failed to enable BULK_OUT EP ret = %d\n",
			      __func__, result);
		}
		...
		if (result != 0) {
			pr_err("%s: failed to enable INTR_OUT EP ret = %d\n",
				__func__, result);
		}
		...
		for (;;) {
			...
			if (!req)
				break;
			if (ether_queue_out(req)) {
				pr_err("%s: ether_queue_out failed\n",
					__func__);
				break;
			}
		}
...
{
	pr_info("usbnet_set_alt intf: %d alt: %d\n", intf, alt);
	do_set_config(1);
			...
			const struct usb_ctrlrequest *ctrl)
{
	...
	if (rc >= 0) {
		...
		if (rc < 0)
			pr_err("usbnet setup response error\n");
	}
...
int usbnet_bind_config(struct usb_configuration *c)
{
	...
	pr_info("Gadget USB: usbnet_bind_config\n");
	...
	if (ret) {
		pr_err("USBNET -  Can't register misc enable device %d \n",
			MISC_DYNAMIC_MINOR);
		goto err1;
	}
