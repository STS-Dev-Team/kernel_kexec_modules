...
struct ovl_disp_size {
	int w;
	int h;
};

static struct ovl_disp_size disp_size_override[] = {
{0, 0}, {0, 0}, {0, 0}, {0, 0} };

static void omap_vout_get_disp_size(struct omap_vout_device *vout,
							int *w, int *h)
{
	...
	if (disp_size_override[ovl->id].w != 0) {
		*w = disp_size_override[ovl->id].w;
		*h = disp_size_override[ovl->id].h;
	} else if (ovl->manager->device) {
		*w = ovl->manager->device->panel.timings.x_res;
		*h = ovl->manager->device->panel.timings.y_res;
	} else { /* Assume WB */
		...
		if (wb) {
			...
			*h = wb->info.width;
			...
			printk(KERN_ERR VOUT_NAME "No Device or Panel\n");
...
void omap_vout_override_disp_size(enum omap_plane ovl, int w, int h)
{
	switch (ovl) {
	case OMAP_DSS_GFX:
	...
	case OMAP_DSS_VIDEO3:
		disp_size_override[ovl].w = w;
		disp_size_override[ovl].h = h;
		break;
	default:
		break;
	};
}
EXPORT_SYMBOL(omap_vout_override_disp_size);
				       ...
				       struct omap_overlay_info *info)
{
	...
	omap_vout_get_disp_size(vout, &dpy_w, &dpy_h);
...
#ifndef CONFIG_ARCH_OMAP4
		info->pos_y = (dpy_h - win->w.width) - win->w.left;
	...
	case dss_rotation_180_degree:
#ifndef CONFIG_ARCH_OMAP4
		info->pos_x = (dpy_w - win->w.width) - win->w.left;
		info->pos_y = (dpy_h - win->w.height) - win->w.top;
#endif
		break;
	...
	case dss_rotation_270_degree:
		...
		info->pos_x = (dpy_w - win->w.height) - win->w.top;
#endif
		break;
			...
			struct v4l2_format *f)
{
	...
	if (!ovl->manager)
		...
		vout->fbuf.fmt.height = dpy_h;
		vout->fbuf.fmt.width = dpy_w;
        } else {
		struct omap_writeback *wb;
		wb = omap_dss_get_wb(0);
		if (wb) {
			...
			struct v4l2_format *f)
{
	...
	if (dev_buf_type & OMAP_FLAG_IDEV)
		...
		if (!cpu_is_omap44xx() && rotate_90_or_270(vout)) {
			vout->fbuf.fmt.height = dpy_w;
			vout->fbuf.fmt.width = dpy_h * multiplier;
		} else {
			vout->fbuf.fmt.height = dpy_h * multiplier;
			vout->fbuf.fmt.width = dpy_w;
		}
        } else {
		struct omap_writeback *wb;
