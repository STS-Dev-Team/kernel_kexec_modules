...
/* Currently set number of pages to reserve */
static unsigned long reserve_page_cap = TMM_DEFAULT_PAGE_CAP;
...
static int tmm_pat_set_free_page_stack_max(struct tmm *tmm,
					 s32 num_pages, bool alloc)
{
	...
	while ((alloc) && (count < reserve_page_cap)) {
		if (fill_page_stack(&pvt->free_list))
			break;
	}
...
struct tmm *tmm_pat_init(u32 pat_id, u32 *dmac_va, u32 dmac_pa)
{
	...
	if (pvt->dummy_pg) {
		...
		tmm->set_free_page_stack_max = tmm_pat_set_free_page_stack_max;
