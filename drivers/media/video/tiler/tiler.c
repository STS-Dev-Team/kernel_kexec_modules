...
static s32 tiler_free_page_max = -1;
static int tiler_free_page_stack_max_set(const char *val,
					struct kernel_param *kp)
{
	int r = param_set_long(val, kp);
	if (!r)
		r = tmm_set_free_page_stack_max(TMM(TILFMT_PAGE),
						tiler_free_page_max, false);
	return r;
}

module_param_call(free_page_max, tiler_free_page_stack_max_set, param_get_long,
					&tiler_free_page_max, 0644);
...
static s32 __init tiler_init(void)
{
	...
	*/
	...
	tmm_set_free_page_stack_max(TMM(TILFMT_PAGE), -1, true);
