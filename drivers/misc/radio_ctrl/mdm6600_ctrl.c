...
static void mdm_ctrl_powerup(void)
{
	...
	pr_debug("%s: starting modem initial status %s [0x%x] bootmode %d",
	...
	if (!bp_status)
		...
		platform_device_unregister(
			mdm_ctrl.pdata->mapphone_bpwake_device);

		mdm_ctrl.pdata->mapphone_bpwake_device =
			platform_device_alloc("mapphone_bpwake", -1);
		if (mdm_ctrl.pdata->mapphone_bpwake_device) {
			platform_device_add(
				mdm_ctrl.pdata->mapphone_bpwake_device);
		} else
			pr_err("%s: FAILED to alloc bpwake_device\n", __func__);
	}
