...
#include "../../arch/arm/mach-omap2/cm44xx.h"
		...
		unsigned int count)
{
	...
	up->console_writing = 1;
	if ((__raw_readl(OMAP4430_CM_L4PER_UART3_CLKCTRL) &
			OMAP4430_IDLEST_MASK) != 0)
		goto console_write_end;
...
static int serial_omap_probe(struct platform_device *pdev)
{
	...
	up->console_writing = 0;
	up->webtop_port = omap_up_info->webtop_port;
	up->console_writing = 0;
...
int omap_uart_active(int num, u32 timeout)
{
	...
	if (status & UART_LSR_DR) {
		if ((up->webtop_port || up->console_port) &&
			(!(iir & UART_IIR_RLSI) && (iir & UART_IIR_NO_INT)))
			serial_omap_clear_fifos(up);
		else
			return 1;
	}
