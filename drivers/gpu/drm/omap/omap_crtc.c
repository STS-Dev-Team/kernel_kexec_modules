			       ...
			       struct drm_framebuffer *old_fb)
{
	...
	omap_crtc->info.color_mode = priv->color_mode;
...
static int commit (struct drm_crtc *crtc)
{
	...
	/* special hack to center underscan resolutions: */
	if (ovl && ovl->manager && ovl->manager->device &&
			ovl->manager->device->driver) {
		struct omap_dss_device *dssdev = ovl->manager->device;
		u16 xres, yres;

		dssdev->driver->get_resolution(dssdev, &xres, &yres);

		if (xres > info->out_width) {
			info->pos_x = (xres - info->out_width) / 2;
		}

		if (yres > info->out_height) {
			info->pos_y = (yres - info->out_height) / 2;
		}
	}
