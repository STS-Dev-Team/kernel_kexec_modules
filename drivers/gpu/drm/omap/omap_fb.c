...
#include <linux/notifier.h>  /* <-- this should be in tiler.h */
...
static int allocate_vram(struct drm_framebuffer *fb, int idx, int size)
{
	...
	if (paddr == ~0) {
		DBG("deferred alloc of %d bytes from TILER for fb %d", size, idx);
		omap_fb->size = size;
		omap_fb->use_tiler = true;
		return 0;
	} else if (paddr) {
...
int omap_framebuffer_open(struct drm_framebuffer *fb)
{
	struct omap_framebuffer *omap_fb = to_omap_framebuffer(fb);
	struct drm_device *dev = fb->dev;

	if (!omap_fb->use_tiler)
		return 0;
	...
	if (omap_fb->cnt == 1) {
		...
		ret = tiler_alloc(TILFMT_PAGE, omap_fb->size, 1,
				(u32 *)&omap_fb->paddr);
		if (ret) {
			dev_err(dev->dev, "could not allocate tiler fb\n");
			return ret;
		}
		DBG("allocating tiler fb: %d bytes, fb=%p", omap_fb->size, fb);

		omap_fb->vaddr = ioremap_wc(omap_fb->paddr, omap_fb->size);
		memset(omap_fb->vaddr, 0, omap_fb->size);
	}

	return 0;
}

int omap_framebuffer_release(struct drm_framebuffer *fb)
{
	struct omap_framebuffer *omap_fb = to_omap_framebuffer(fb);
	struct drm_device *dev = fb->dev;
	...
	if (omap_fb->cnt == 0) {
		...
		DBG("disabling attached CRTCs: fb=%p", fb);
		...
		DBG("deallocating tiler fb: fb=%p", fb);

		if (!omap_fb->paddr) {
			DBG("tiler memory address to be freed is NULL");
			return 0;
		}

		ret = tiler_free(omap_fb->paddr);
		if (ret) {
			dev_err(dev->dev, "could not free tiler fb\n");
			return ret;
		}
