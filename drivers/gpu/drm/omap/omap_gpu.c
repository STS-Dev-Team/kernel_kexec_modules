...
struct omap_gpu_notifier_block {
		...
		unsigned long evt, void *arg)
{
	...
	case OMAP_DSS_HOTPLUG_DISCONNECT: {
		struct omap_gpu_notifier_block *onb =
				container_of(nb, struct omap_gpu_notifier_block, base);
		struct drm_device *dev = onb->dev;
...
static int omap_modeset_init(struct drm_device *dev)
{
	...
	int create_connector(struct omap_dss_device *dssdev) {
		static struct omap_gpu_notifier_block *notifier;
		...
		notifier = kzalloc(sizeof(struct omap_gpu_notifier_block), GFP_KERNEL);
		notifier->base.notifier_call = omap_gpu_notifier;
		notifier->dev = dev;
		omap_dss_add_notify(dssdev, &notifier->base);
	...
	if (pdata) {
		...
		priv->color_mode = pdata->color_mode;
	...
	if (!priv->color_mode)
		priv->color_mode = OMAP_DSS_COLOR_ARGB32;
...
static void dev_postclose(struct drm_device *dev, struct drm_file *file)
{
	...
	if (priv->fb)
		omap_framebuffer_release(priv->fb);
