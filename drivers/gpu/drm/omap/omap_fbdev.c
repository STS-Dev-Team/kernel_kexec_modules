...
static int omap_fbdev_open(struct fb_info *fbi, int user)
{
	struct drm_fb_helper *helper = get_fb(fbi);

	if (!helper) {
		DBG("drm fb helper is null");
		return -1;
	}

	DBG("open: fbi=%p", fbi);

	return omap_framebuffer_open(helper->fb);
}

int omap_fbdev_release(struct fb_info *fbi, int user)
{
	struct drm_fb_helper *helper = get_fb(fbi);
	...
	DBG("release: fbi=%p", fbi);

	return omap_framebuffer_release(helper->fb);
}

/* NOTE: normally we'd let drm_fb_helper handle this, but we run into
 * issues with the current version of 3pdd (omaplfb).. since we configure
 * the DRM device with DRM APIs, but flip with fbdev APIs, we run into
 * a problem that the drm_fb_helper code gets out of sync.. if it isn't
 * the one that configures the fb -> crtc -> encoder -> connector chain,
 * it will think the device has no connector.
 *
 * The proper solution is to finish converting pvr driver to use only
 * DRM APIs and no fbdev APIs.  For now this hack should suffice.
 */
extern bool omap_crtc_first;
int omap_fbdev_pan_display(struct fb_var_screeninfo *var,
			      struct fb_info *fbi)
{
	struct drm_fb_helper *helper = get_fb(fbi);
	...
	DBG("pan_display: %d,%d, fbi=%p", var->xoffset, var->yoffset, fbi);
	if (omap_crtc_first) {
		drm_fb_helper_pan_display(var, fbi);
		return 0;
	}
	...
	while ((connector = omap_framebuffer_get_next_connector(fb, connector))) {
		...
		if (connector->encoder && connector->encoder->crtc) {
			/* note: this isn't right for multi-head setups in virtual
			 * display mode (where each crtc has a different x/y offset)..
			 * but we don't care about that for webtop..  if you care
			 * about multi-head, don't use this hack, instead re-write
			 * 3pdd to do flipping properly ;-)
			 */
			...
			crtc_funcs->mode_set_base(crtc, crtc->x, crtc->y, crtc->fb);
		}
	}
...
static struct fb_ops omap_fb_ops = {
	...
	.fb_open = omap_fbdev_open,
	.fb_release = omap_fbdev_release,
