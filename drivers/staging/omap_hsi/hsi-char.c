...
static int __init hsi_char_init(void)
{
	...
	if (IS_ERR(hsi_char_class)) {
		pr_err("%s: Something wrong in class_create", __func__);
	...
	for (i = 0; i < HSI_MAX_CHAR_DEVS; i++) {
		pr_info("channels_map[%d] = %d\n" , i, channels_map[i]);
		if ((int)(channels_map[i] - 1) >= 0) {
			if (IS_ERR(device_create(hsi_char_class, NULL,
						MKDEV(hsi_char_major,
						channels_map[i] - 1),
						NULL, "hsi%d" ,
						channels_map[i] - 1))) {
				...
				unregister_chrdev_region(hsi_char_dev,
					HSI_MAX_CHAR_DEVS);
				class_destroy(hsi_char_class);
				return -EFAULT;
			}
