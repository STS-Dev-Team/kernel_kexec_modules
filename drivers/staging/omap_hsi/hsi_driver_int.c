...
/* hsi_clear_dsp_wake_up - SW WA for hardcoded wakeup dependency from HSI to DSP
*
...
static int hsi_clear_dsp_wake_up(struct hsi_dev *hsi_ctrl)
{
	...
	/* Wait until DSP Power Domain is functional */
...
static void do_hsi_tasklet(unsigned long hsi_port)
{
	...
	/* Force a DSP SW wakeup/sleep only if Swakeup is asserted to MPU. */
	/* This is achieved by checking if HSI clocks were disabled when  */
	/* HSI interrupt has occured. */
	/* HW Bug ref is HSI-C1BUG00106 : dsp swakeup generated by HSI same */
	/* as mpu swakeup */
