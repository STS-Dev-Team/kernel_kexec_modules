...
static void qcnet_urbhook(struct urb *urb)
{
	...
	usb_free_urb(urb);
	wake_up_interruptible(&worker->urb_wq);
}
...
static int qcnet_worker(void *arg)
{
	...
	unsigned int resume_fail_count = 0;
	if (!worker) {
	...
	while (1) {
		...
		if (status < 0) {
			if (printk_ratelimit()) {
				ERR("unable to autoresume: %d, dpm_state: %d udev_state %d\n",
					status, iface->dev.power.status, usbdev->state);
			}
			...
			if(++resume_fail_count > 100) {
				ERR("unable to autoresume: %d, power status: %d PANIC\n",
		...
		if (status < 0) {
			...
			status = wait_event_interruptible_timeout(worker->urb_wq,
							(worker->active == NULL),
							5*HZ);
			if (!status) {
				ERR("URB timedout: %d. Kill Worker\n", status);
				qcnet_killactive(worker);
			}
		}
		kfree(req);
...
static int qcnet_open(struct net_device *netdev)
{
	...
	init_waitqueue_head(&dev->worker.urb_wq);
...
int qcnet_probe(struct usb_interface *iface, const struct usb_device_id *vidpids)
{
	...
	usbnet->udev->autosuspend_delay = HZ/2;
