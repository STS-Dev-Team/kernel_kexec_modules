				...
				enum cpcap_adc_bank0 chan)
{
	...
	dev_err(&(cpcap->spi->dev), "CPCAP_REG_ADCAL1 : 0x%4X\n",
		(short)cal_data);
	dev_err(&(cpcap->spi->dev), "CHG_ISENSE: %d\n",
		((short)cal_data * -1) + 512);

	cal_data = 0;
	cpcap_regacc_read(cpcap, CPCAP_REG_ADCAL2, &cal_data);
	dev_err(&(cpcap->spi->dev), "CPCAP_REG_ADCAL2 : 0x%4X\n",
		(short)cal_data);
	dev_err(&(cpcap->spi->dev), "BATTI_ADC OFFSET: %d\n",
		((short)cal_data * -1) + 512);
}
