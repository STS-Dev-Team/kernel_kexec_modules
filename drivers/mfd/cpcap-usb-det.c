...
static unsigned char vbus_valid_adc_check(struct cpcap_usb_det_data *data)
{
	...
	dev_err(&data->cpcap->spi->dev,
		"ADC_CHG_ISENSE - VLD_CHK : %d\n",
		req.result[CPCAP_ADC_CHG_ISENSE]);
	dev_err(&data->cpcap->spi->dev,
		"ADC_VBUS - VLD_CHK : %d\n",
		req.result[CPCAP_ADC_VBUS]);
	dev_err(&data->cpcap->spi->dev,
		"ADC_BATTP - VLD_CHK : %d\n",
	req.result[CPCAP_ADC_BATTP]);
	return ((req.result[CPCAP_ADC_CHG_ISENSE] < 50) &&
		(req.result[CPCAP_ADC_VBUS] <
		(req.result[CPCAP_ADC_BATTP]+100))) ? false : true;
}

static unsigned char print_adc(struct cpcap_usb_det_data *data)
{
	struct cpcap_adc_request req;
	int ret;

	dev_info(&data->cpcap->spi->dev, "CPCAP: print_adc\n");
	req.format = CPCAP_ADC_FORMAT_CONVERTED;
	req.timing = CPCAP_ADC_TIMING_IMM;
	req.type = CPCAP_ADC_TYPE_BANK_0;
...
static void detection_work(struct work_struct *work)
{
	...
	case WHISPER_SMART:
		...
		if ((!(data->sense & CPCAP_BIT_SESSVLD_S)) ||
			...
			data->state = WHISPER_SMART;
			/* ID Pin should be released for SPDIF.
		       ...
		       unsigned long cmd)
{
	...
	pr_cpcap_usb_det(STATUS, "%s: ioctl cmd = 0x%08lx\n", __func__, cmd);
	...
	if ((data->state == CHARGER) ||
		...
		if (value) {
			...
			data->whisper_auth = AUTH_PASSED;
			cpcap_irq_unmask(data->cpcap, CPCAP_IRQ_VBUSVLD);
		} else if (!(cmd & CPCAP_WHISPER_ENABLE_UART)) {
			data->whisper_auth = AUTH_FAILED;
		}
...
static int __init cpcap_usb_det_probe(struct platform_device *pdev)
{
	...
	for (i = CPCAP_REG_START; i < CPCAP_NUM_REG_CPCAP; i++)	{
		dev_err(&pdev->dev,
			" Reading cpcap register number: %4d:",
			register_info[i].address);
		retval = cpcap_regacc_read(data->cpcap, i, &value);
		if (retval)
			dev_err(&pdev->dev,
				"attempted read fail register number: %4d\n",
				register_info[i].address);
		else
			dev_err(&pdev->dev, "Register content: 0x%04X\n",
				value);
	}
