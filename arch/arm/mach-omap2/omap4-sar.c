...
u32 usb_sar_data[(USB_SAR_AREA_END-USB_SAR_AREA_START)/4];
	...
	for (i = 0; i < nb_regs; i++) {
		if (omap4_sar_modules[(sar_layout_table[i][MODULE_ADDR_IDX])]) {
			...
			for (j = 0; j < size; j++) {
				...
				if (sar_layout_table == sar_ram1_layout &&
...
static int omap4_sar_not_accessible(void)
{
	...
	if ((usbhost_state == (OMAP4430_STBYST_MASK | OMAP4430_IDLEST_MASK)) &&
		...
		pr_err("%s: usbhost_state %x, usbtll_state %x\n", __func__,
			usbhost_state, usbtll_state);
		return -EBUSY;
	}
}
