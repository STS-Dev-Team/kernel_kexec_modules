...
static int power_off_notify(struct notifier_block *this,
			unsigned long event, void *ptr)
{
	u32 mask, val;
	...
	if ((event == SYS_POWER_OFF) ||
		...
		val = 1 << OMAP4430_MEMIF_STATDEP_SHIFT;
		cm_rmw_mod_reg_bits(mask, val, OMAP4430_CM1_MPU_MOD,
...
static struct notifier_block power_off_block = {
	.notifier_call = power_off_notify,
	.priority = 1,
};
...
static void __init mapphone_init(void)
{
	...
	register_reboot_notifier(&power_off_block);
}
