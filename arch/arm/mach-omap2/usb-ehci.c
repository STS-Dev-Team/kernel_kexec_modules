...
struct uhhtll_hcd_omap {
	...
	struct wake_lock 		ehci_wakelock;
	struct wake_lock 		ohci_wakelock;
};
...
static int uhhtll_hcd_omap_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&omap->ehci_wakelock, WAKE_LOCK_SUSPEND,
			"ehci_wakelock");
	wake_lock_init(&omap->ohci_wakelock, WAKE_LOCK_SUSPEND,
			"ohci_wakelock");
#ifdef CONFIG_PM
...
static int uhhtll_drv_suspend(enum driver_type drvtype)
{
	...
	if ((drvtype == OMAP_EHCI) &&
		...
		if (!test_bit(USBHS_EHCI_SUSPENED, &omap->event_state)) {
			...
			dev_dbg(&omap->pdev->dev, "Disable EHCI Controller \n");
			...
			save_usb_sar_regs();
			wake_unlock(&omap->ehci_wakelock);
		}
	} else if ((drvtype == OMAP_OHCI) &&
		test_bit(USBHS_OHCI_LOADED , &omap->event_state)) {
		if (!test_bit(USBHS_OHCI_SUSPENED, &omap->event_state)) {
			...
			dev_dbg(&omap->pdev->dev, "Disable OHCI Controller \n");
			usbhs_disable(omap, 0);
			...
			wake_unlock(&omap->ohci_wakelock);
		}
	} else
		ret = -EINVAL;
...
static int uhhtll_drv_resume(enum driver_type drvtype)
{
	...
	if ((drvtype == OMAP_EHCI) &&
		...
		if (test_and_clear_bit(USBHS_EHCI_SUSPENED,
			&omap->event_state)) {
			wake_lock_timeout(&omap->ehci_wakelock,
					MAX_WAKELOCK_TIME);
			usbhs_phy_safe_local = usbhs_phy_safe;
			usbhs_ehci_devoff_resume(omap);
			dev_dbg(&omap->pdev->dev, "Enable EHCI Controller \n");
		...
		if (test_and_clear_bit(USBHS_OHCI_SUSPENED,
			&omap->event_state)) {
			dev_dbg(&omap->pdev->dev, "Enable OHCI Controller \n");
			wake_lock_timeout(&omap->ohci_wakelock,
					MAX_WAKELOCK_TIME);
			if (pdata->ohci_phy_suspend) {
