...
static int __init activate_uart(void)
{
	int i;
	u16 tmp = 0;
	struct omap_hwmod* (*omap_hwmod_lookup_ptr)(unsigned char *);
	struct omap_hwmod *oh;

	int (*cpcap_direct_misc_write_ptr)(unsigned short reg, unsigned short value,
						unsigned short mask);

	omap_hwmod_lookup_ptr = (void *)kallsyms_lookup_name("omap_hwmod_lookup");

	if(!omap_hwmod_lookup_ptr)
	{
		printk(KERN_ERR "Could not get omap_hwmod pointer\n");
		return -1;
	}

	cpcap_direct_misc_write_ptr = (void *)kallsyms_lookup_name("cpcap_direct_misc_write");

	if(!cpcap_direct_misc_write_ptr)
	{
		printk(KERN_ERR "Could not get cp cap pointer\n");
		return -1;
	}

	oh = omap_hwmod_lookup_ptr("mcspi1");
...
#else
	cpcap_direct_misc_write_ptr(897, 0x0F07, 0xFFFF);
 	cpcap_direct_misc_write_ptr(411, 0xFFFF, 0xFFFF);	
#endif
	/* Step 3:
