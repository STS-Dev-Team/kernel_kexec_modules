...
#define COPY_ANSWERS_FAILURE 0xF123F123
...
static int tf_copy_answers(struct SCXLNX_COMM *pComm)
{
	u32 nFirstAnswer;
	...
	if (test_bit(SCXLNX_COMM_FLAG_L1_SHARED_ALLOCATED, &(pComm->nFlags))) {
		...
		while (nFirstAnswer != nFirstFreeAnswer) {
			...
			if (pAnswerStructureTemp->pAnswer != NULL && nCommandSize < 0x200000)
			{
			...
			{
				printk(KERN_ERR
				"tf_copy_answers: bad message parameters %d", nCommandSize);
				status = COPY_ANSWERS_FAILURE;
				break;
			}
		}
		spin_unlock(&(pComm->lock));
...
copy_answers:
	...
	if (tf_copy_answers(pComm) == COPY_ANSWERS_FAILURE)
		return COPY_ANSWERS_FAILURE;
	  ...
	  bool bKillable)
{
	...
	if (nError == COPY_ANSWERS_FAILURE)
		goto exit;
